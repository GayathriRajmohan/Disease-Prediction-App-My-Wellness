...
def sign_in_patient(request):
    if request.method == 'POST':
        ...
        if new_password:
            ...
            if user:
                # Set the new password for the user if 'new_password' is provided
...
def forgot_passwordd(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            user.set_password(new_password)
            user.save()
            return redirect('password_success')  # Redirect to a success page
        else:
            return render(request, 'forgot_passwordd.html', {'error': 'Invalid username'})

    return render(request, 'forgot_passwordd.html')
